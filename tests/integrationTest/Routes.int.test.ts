...
})); //convert schedule is not dependent on the routes, so we have mocked it in the integration test here
...
describe("Routes test suite", () => {
  ...
  describe("checking integration with validateData", () => {
    test("valid data should not throw any error", async () => {
      const res = await request(app)
        .post("/notification-profiles")
        .send(mockProfileData);

      expect(res.statusCode).toBe(201); //willreturn success as the savedata function, when called will return success
    });

    test("invalid data should throw error", async () => {
      let mockData = {
        //this data does not contain some required fields
  ...
  describe("Checking integration wiht DBOperations-POST/", () => {
